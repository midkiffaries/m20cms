...
if ($_POST['itemFunction'] == 3 || regExpression($formItem1, 0) && regExpression($formItem2, 0) && regExpression($textBox, 0)) {
	...
	switch ($_POST['itemFunction']) {
		case 1:
			...
			$query = @mysql_query("UPDATE $Table SET title='$formItem1', catagories='$formItem2', content='$textBox', template='0' WHERE id='$formItem0'");						
			break;
		case 2:
			// Create new table entry
			$query = @mysql_query("INSERT INTO $Table (title, catagories, author, content, template, date) VALUES ('$formItem1', '$formItem2', '$formItem3', '$textBox', '0', '$UnixDate')");
			break;
		case 3:
...
while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
	...
	if ($_POST['id'] == $row['id'] && !$errorNoUpdate) {
		...
		if (!$formItem5) $formItem5 = $row['template'];
		...
		$displayTable .= createRow($rowColor, $row['id'], $row['title'], getUsernameId($row['author']), $row['catagories'], convertDate($row['date']));
	}
	// Alternate row color
	$rowCount++;
}
